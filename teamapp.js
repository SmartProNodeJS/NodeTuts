...
var teamapp = express();
teamapp.use(Session({secret:"NodeJSSecret"}));
// Middle ware to put db object to request
teamapp.use(function(request, res, next){
  if(!request.db)
    request.db = db;
  next();
});
var teamRoute = require('./routes/team.js');
teamapp.use("/team", teamRoute);
// Create teamapplication/x-www-form-urlencoded parser
var urlencodedParser = bodyParser.urlencoded({ extended: false })
teamapp.use(express.static("public/"));
teamapp.use(cookies());
teamapp.set("views",path.join(__dirname,"views"));
teamapp.set("view engine","pug"); 

teamapp.get('/', urlencodedParser, function (req, res) {
...
teamapp.get('/sports',function(req, res) {
...
teamapp.get('/sport/:name',function(req, res) {
...
teamapp.post('/sport',urlencodedParser, function(req, res) {
...
var ioHttp = http.Server(teamapp);
...
teamapp.use(handle404Pages);

var server = ioHttp.listen(8081, function () {
    var host = server.address().address,
    port = server.address().port;
    console.log("Example teamapp listening at http://%s:%s", host, port);
})
