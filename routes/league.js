...
route.get('/:id',  function (req, res) {
    ...
    if(idCheck.test(id)) {
      ...
      var newleague= {"_id":new ObjectID(),"league_name":"","number_match":"","date_start":"","date_end":"","sponsors_name":"","organiser_name":"","sport":""}
         sports.find({},{}, function(err, sport_list){
...
route.post('/',urlencodedParser,  function (req, res) {
  console.log("Request.body = "+JSON.stringify(req.body));
    ...
    league.sponsors_name = req.body.sponsors_name;
    league.organiser_name = req.body.organiser_name;
...
route.post('/addmatch', urlencodedParser, function (req, res){
  ...
  if(JSON.parse(req.body.league_match_list)){
    ...
    data.forEach(function(it){
      matchCol.find({"_id": it}, {}, function(err, m){
        if(err){
          console.log(" Error: "+JSON.stringify(err));
        }
        else{
          leagueCol.update({ "_id": id},{ $push: {"match_list": m[0]}});
          res.json(true);
        }
      });
    });
